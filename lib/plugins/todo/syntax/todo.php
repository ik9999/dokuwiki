...
class syntax_plugin_todo_todo extends DokuWiki_Syntax_Plugin {
    ...
    protected function createTodoItem(&$renderer, $id, $data) {
        ...
        switch($data['checked']."")
        {
            case "1" : $checked = ' checked="checked"'; $delll = true; break;
            case "2" : $medium = ' indeterminate="indeterminate"'; break;
            default: break;
        }

        if($data['checkbox']) {
            $return = '<input data="'.$data['checked'].'" type="checkbox" class="todocheckbox tristate"'
        ...
        if($delll && $this->getConf("Strikethrough")) {
            $return .= '<del>';
        }
